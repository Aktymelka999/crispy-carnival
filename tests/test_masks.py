...
class TestMasks:
    @pytest.mark.parametrize(
        "card_num,expected", [("1234567890123456", "1234 56** **** 3456"), ("4444333322221111", "4444 33** **** 1111")]
    )
    ...
    @pytest.mark.parametrize(
        "account_num,expected", [("12345678901234567890", "**7890"), ("98765432109876543210", "**3210")]
    )
    def test_mask_account_number_valid(self, account_num, expected):
        assert mask_account_number(account_num) == expected

    @pytest.mark.parametrize(
        "invalid_account", ["123456789012", "123456789012345678901234", "123a567b901c345d678e", ""]
    )

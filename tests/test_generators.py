...
from src.generators import card_number_generator, description_generator, filter_by_currency, transaction_descriptions
...
def test_filter_by_currency_single_match():
    single_tx = [{"id": 1, "operationAmount": {"currency": "EUR", "amount": 100}}]
...
def test_filter_by_currency_none_values():
    transactions_with_none = [None, {"id": 1, "operationAmount": {"currency": "USD", "amount": 100}}, None]
...
def test_filter_by_currency_skip_none_and_non_dict():
    transactions = [None, "not a transaction", 123, {"id": 1, "operationAmount": {"currency": "USD", "amount": 100}}]
    result = list(filter_by_currency(transactions, "USD"))
    assert len(result) == 1
...
def test_description_generator_with_description():
    transactions = [{"id": 1, "description": "Покупка в магазине"}, {"id": 2, "description": "Перевод другу"}]
...
def test_description_generator_without_description():
    transactions = [{"id": 1}, {"id": 2, "amount": 100}]
    result = list(description_generator(transactions))
...
def test_card_number_generator_normal_range():
    ...
    expected = ["1234567890123456", "1234567890123457", "1234567890123458"]
...
def test_card_number_generator_large_range():
    ...
    expected = ["1000000000000000", "1000000000000001", "1000000000000002"]
    assert result == expected
...
def test_transaction_descriptions_without_descriptions():
    ...
    transactions = [{"id": 1, "amount": 100}, {"id": 2, "amount": 200, "currency": "RUB"}, {"id": 3}]
    result = list(transaction_descriptions(transactions))
...
def test_transaction_descriptions_mixed():
    ...
    result = list(transaction_descriptions(transactions))
    expected = ["Перевод другу", "Описание отсутствует", "Оплата интернета", "Описание отсутствует"]
    assert result == expected
...
def test_transaction_descriptions_generator_behavior():
    """Тест: проверка ленивой загрузки (генератора)."""
    transactions = [{"id": 1, "description": "Первая транзакция"}, {"id": 2, "description": "Вторая транзакция"}]
